...
model.W_Ogrid = Set(initialize=list(range(
    1 + 1, len(data.Technologies.columns) + 2)), within=model.In)  # techs w/o grid
...
model.linCapCosts = Param(model.In, model.Out, initialize=data.LinearCost())
...
model.NetPresentValueStor = Param(
    model.Out, domain=NonNegativeReals, initialize=data.StorageNPV())
...
def roofArea_rule(model, roof, demand):
    # DemandData.shape[1]
    return (sum(model.Capacities[roof, d] for d in number_of_demands) <= model.maxSolarArea)


model.roofArea = Constraint(
    model.roof_tech, model.Out, rule=roofArea_rule)  # model.roof_tech
...
for x in range(1, len(dispatch_demands) + 1):
    model.con.add(model.Capacities[CHP_list[x - 1], dispatch_demands[x - 1, 1]] == model.cMatrix[CHP_list[x - 1], dispatch_demands[x - 1, 1]
                                                                                                 ] / model.cMatrix[CHP_list[x - 1], dispatch_demands[x - 1, 0]] * model.Capacities[CHP_list[x - 1], dispatch_demands[x - 1, 0]])
    model.con.add(model.Ytechnologies[CHP_list[x - 1], dispatch_demands[x - 1, 0]]
                  == model.Ytechnologies[CHP_list[x - 1], dispatch_demands[x - 1, 1]])
    model.con.add(model.Capacities[CHP_list[x - 1], dispatch_demands[x - 1, 0]] <=
